...
import { getAcceptedFormats } from '@/lib/get-file-type';
...
const MultiFileUploader = ({ props }: FileUploadProp) => {
  const { uploadedFiles, uploading, handleFileChange, handleRemoveFile } = useFileUpload({
    name: props.name,
    ...
    <div className="grid gap-6">
      <div className="grid gap-3">
        ...
        {props.FileDescription && (
          <p className="text-muted-foreground text-sm">{props.FileDescription}</p>
        )}
      </div>
